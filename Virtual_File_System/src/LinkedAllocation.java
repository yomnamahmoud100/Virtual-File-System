...
public class LinkedAllocation extends Allocation{
    ...
    public boolean createFile(String filePath , int size) {
        ...
        if (exist){
            ...
            for (int j = 0; j < memory.length() && size2 != 0 ; j++) {
                if(memory.charAt(j) == '0'){
                  newMemory[j] ='1';
                  file.fileAllocatedBlocks.add(j);
                  size2 --;
                }
            }
        ...
        if(root.files.size() != 0){
            for (File2 file2 : root.files) {
                ...
                if(size != 0) {
                    ...
                    myWriter.write(file2.filePath + " " + start + " " + end + "\n");
                    int i ;
                    for (i = 0; i < file2.fileAllocatedBlocks.size() - 1; i++){
                        myWriter.write(file2.getAllocatedBlocks().get(i) + " " + file2.getAllocatedBlocks().get(i+1) + "\n");
                    }
                    myWriter.write(file2.getAllocatedBlocks().get(i) + " nil \n");
                }
            }
        }
